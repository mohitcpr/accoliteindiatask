...
class App extends Component {
  ...
  onCountChange = (index, count) => {
    ...
    if (
      (itemList[index].count === 0 && count === -1) ||
      itemList[index].count + count > itemList[index].available
    ) {
      return;
    }
    itemList[index].count = itemList[index].count + count;
    this.setState({
      itemList,
      selectedItem: index.toString() + itemList[index].count
    });
  };
  componentDidMount() {
    this.getItemList();
    ...
    return (
      <View style={styles.mainContainer}>
        <ScrollView style={styles.app}>
          ...
          <ItemList
            itemList={itemList}
            onCountChange={(index, count) => this.onCountChange(index, count)}
...
const styles = StyleSheet.create({
  ...
  mainContainer: { flex: 1, width: Dimensions.get("window").width }
});
